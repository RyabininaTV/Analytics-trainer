...
public class CreateTaskService {
    ...
    public CreateTaskResponse createTask(@Nonnull CreateTaskRequest request) {
        if (!trainersRepository.existsActiveById(request.trainerId())) {
            throw new TrainerNotFoundException(request.trainerId());
        }

        validateTaskPayload(request);

        CreateTaskResponseEntity task = tasksRepository.create(toCreateTaskRequestEntity(request));
    ...
    private static void validateTaskPayload(@Nonnull CreateTaskRequest request) {
    ...
    private static void validateTestTaskPayload(@Nonnull CreateTaskRequest request) {
        ...
        boolean hasCorrectOption = request.options().stream()
                .anyMatch(CreateTaskRequest.CreateTaskOptionRequest::isCorrect);
    ...
    private static void validateErrorFindTaskPayload(@Nonnull CreateTaskRequest request) {
        if (isEmpty(request.errorItems())) {
        ...
        boolean hasErrorItem = request.errorItems().stream()
                .anyMatch(CreateTaskRequest.CreateTaskErrorItemRequest::isError);
    ...
    private static void validateOpenTaskPayload(@Nonnull CreateTaskRequest request) {
        if (!isEmpty(request.options())) {
    ...
    private static CreateTaskRequestEntity toCreateTaskRequestEntity(@Nonnull CreateTaskRequest request) {
        return CreateTaskRequestEntity.builder()
    ...
    private List<CreateTaskOptionResponseEntity> createOptionsIfNeeded(
            Long taskId,
            @Nonnull CreateTaskRequest request
    ) {
        if (!TEST_TASK_TYPE.equals(request.taskType())) {
            return List.of();
        }
    ...
    private static List<CreateTaskResponse.CreateTaskOptionResponse> toOptionResponses(
        ...
        return options.stream()
                .map(option -> CreateTaskResponse.CreateTaskOptionResponse.builder()
                        .id(option.id())
                        .optionText(option.optionText())
                        .isCorrect(option.isCorrect())
                        .build()
    ...
    private static List<CreateTaskResponse.CreateTaskErrorItemResponse> toErrorItemResponses(
        ...
        return errorItems.stream()
                .map(errorItem -> CreateTaskResponse.CreateTaskErrorItemResponse.builder()
                        .id(errorItem.id())
                        .fragmentText(errorItem.fragmentText())
                        .isError(errorItem.isError())
                        .explanation(errorItem.explanation())
